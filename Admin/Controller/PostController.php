...
class PostController extends BaseController
{
    ...
    public function insert()
    {
        ...
        if (empty($_FILES["video"]['name'])){
            $errorsPost["trailer"] = "Trailer không được để trống!";
            $statusPost = false;
        }
        $uploadVideo = new Uploadfile();
        $uploadFile = $uploadVideo->fileUpload("video", "video", array("mp4", "mkv", "vob"),100);
    ...
    public function update()
    {
        ...
        if (empty($_FILES["video"]['name'])){
            $errorsUpdate["trailer"] = "Trailer không được để trống!";
            $statusUpdate = false;
        }
        $uploadVideo = new Uploadfile();
        $uploadFile = $uploadVideo->fileUpload("video", "video", array("mp4", "mkv", "vob"),100);
        if (!$uploadFile[0]){
            $errorsUpdate["errorTrailer"]=$uploadFile[1];
            $statusUpdate = false;
